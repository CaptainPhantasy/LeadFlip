/**
 * Discovery Scan Job Handler
 *
 * Searches for businesses in a specific location and service category using Google Places API.
 * Filters results by quality criteria and inserts qualified businesses into the database.
 */

import { Job } from 'bullmq';
import { createClient } from '@supabase/supabase-js';
import type { DiscoveryScanJob } from './queues';
...
/**
 * Process discovery scan job
 */
export async function processDiscoveryScan(job: Job<DiscoveryScanJob>) {
  const { zipCode, serviceCategory, radius } = job.data;

  console.log(`[Discovery Scan] Starting scan for ${serviceCategory} in ${zipCode} (${radius} miles)`);

  try {
    // Step 1: Get coordinates for ZIP code (simplified - in production, use geocoding API)
    ...
    for (const result of results) {
      try {
        ...
        if (error) {
          ...
          discoveredCount++;
          console.log(`[Discovery Scan] Added: ${details.name} (${distance} miles, ${details.rating} stars)`);
        }
    ...
    console.error(`[Discovery Scan] Job failed:`, error);
...
async function getZipCodeCoordinates(zipCode: string): Promise<{ lat: number; lng: number } | null> {
  ...
  const zipCoordinates: Record<string, { lat: number; lng: number }> = {
    ...
    // ========================================
    '46702': { lat: 41.2234, lng: -85.4125 }, // Angola
    '46992': { lat: 40.7734, lng: -86.1458 }, // Logansport
    '47421': { lat: 38.8606, lng: -86.4528 }, // Bedford
    '47520': { lat: 39.3547, lng: -86.4708 }, // Bedford (North)
  };
